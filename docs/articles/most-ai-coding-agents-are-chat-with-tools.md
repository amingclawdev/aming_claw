...
**Weak test binding / graph self-repair**

- Failure: a TypeScript node was falsely credited with unrelated Python tests as
  strong coverage.
- Surface that caught it: relationship / impact + constraint.
- Evidence: the graph exposed the weak binding, the AI proposal went to review
  instead of mutating truth, the rule moved into the test fan-in path, a
  regression fixture was added, and full reconcile materialized the corrected
  graph.
- Links: [semantic rule](https://github.com/amingclawdev/aming-claw/blob/main/docs/config/semantic-enrichment.md),
  [fixture](https://github.com/amingclawdev/aming-claw/blob/main/docs/fixtures/external-governance-demo/l4-smoke-fixture.md).

**Observer Reminder Echo**

- Failure: a hook reminder carried no business payload, so a worker could have
  claimed context it never received.
- Surface that caught it: intent + process + constraint.
- Evidence: the observer claims the command before the payload is visible, and
