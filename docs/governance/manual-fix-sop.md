...
| R24 | Route context consumption gate | `observer_led_parallel_lanes`, `mf_parallel.v1`, P0/P1 governance/routing/runtime/backlog/permission work | Route context text is not enough. Precommit and backlog-close gates require passing timeline evidence for `route_context`, `route_action_precheck`, `bounded_implementation_worker_dispatch`, and `mf_subagent_startup`, all carrying the same `route_context_hash`, `prompt_contract_id`, and `prompt_contract_hash`. Generic waivers or advisory notes without the matching route identity do not satisfy this gate. |
