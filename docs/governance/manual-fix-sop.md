...
| R20 | Route-token mutation gate | Implementation-worker dispatch, mutation-bearing task completion, merge queue/result/live-merge mutations, backlog close, and release gate calls | Protected external mutations MUST include `route_token` evidence with route context hash, prompt contract id, caller role, allowed action, matching scope, expiry, and evidence refs. An explicit `route_waiver` is allowed only for accepted manual-fix or same-worktree exceptions with reason and timeline evidence. Read-only backlog/status/graph queries and dry-run merge planning do not require a token. |
| R21 | Asset binding/drift two-line model | Any doc/test/config bind, unbind, asset impact, or drift claim | Binding truth and drift observation MUST stay on separate audit lines. Binding relationship changes are source-controlled append-only commands, normally governance-hint bind/unbind events, then reconcile materializes graph secondary/test/config, file inventory effective state, asset projection, and binding events. File/hash/drift/impact state is observed DB evidence written by reconcile, gate, or workflow worker from git diff plus accepted bindings. Changed bound assets covered by contract/gate may be recorded as `not_drifted` with gate evidence; unchanged bound assets impacted by related source/config changes MUST become `suspected`/`impact_pending` until observer, user, or AI-assisted review resolves them. Do not directly hand-write trusted accepted binding rows into DB as a substitute for source-controlled binding evidence. |
| R22 | Observer/judge no-direct-code topology gate | Governed nontrivial implementation work | Observer/judge MUST NOT directly write implementation code. If a local route/precheck provider is configured, observer MUST resolve it through Aming-owned route/precheck contracts and record provider id, version, and hash evidence before implementation planning. Source-controlled Aming prompts, docs, and templates MUST NOT name private provider systems or provider-specific tool names. Nontrivial implementation MUST be dispatched to bounded `mf_sub`/worker lanes with target files, tests or a recorded no-test/E2E decision, worktree/fence evidence, and review evidence. The only direct observer mutation exception is tiny deterministic scope with an explicit reason, allowed files, exact dirty-scope match evidence, and observer timeline event before mutation. |
| R23 | Route topology and independent verification gate | Route/precheck, governance/routing/runtime/backlog/permission changes | `route.prompt_alert_bundle` MUST classify work as `lightweight_single_lane` for small deterministic changes or `observer_led_parallel_lanes` for high-risk governance/routing/runtime/backlog/permission work. The bundle MUST expose `selected_topology`, `recommended_topology`, `required_lanes`, `reason_codes`, `observer_authorities`, and a bounded worker prompt contract. P0/P1 governance/routing/permission/runtime changes MUST have independent verification lane evidence before `workflow.merge` or `backlog.close` can pass. Observer remains the sole authority for merge, redeploy, graph reconcile, backlog close, waivers, and merge queue mutation. |
| R24 | Route context consumption gate | `observer_led_parallel_lanes`, `mf_parallel.v1`, P0/P1 governance/routing/runtime/backlog/permission work | Route context text is not enough. Before local writes, run the local `route.pre_mutation` precheck or an equivalent `route.action_precheck` service gate. P0/cross-module/parallel implementation actions require machine route identity fields `route_context_hash` and `prompt_contract_id`, caller role, allowed actions, blocked actions, required lanes, required evidence, and public-safe `visible_injection_manifest_hash` or `visible_injection_manifest`. Precommit and backlog-close gates still require passing timeline evidence for `route_context`, `route_action_precheck`, `bounded_implementation_worker_dispatch`, and `mf_subagent_startup`, all carrying the same required route identity. `prompt_contract_hash` may be supplied for propagation/comparison, but is not required route identity. Generic waivers, advisory route text, or `preflight_check` output alone do not satisfy this gate. If the route provider is unavailable, transport is closed, or route evidence is stale, mutation/write actions must block as `blocked_route_context_unavailable`. |
...
#### R22.1 Observer/Judge No-Direct-Code and Tiny Deterministic Exception
...
If a local route/precheck provider is configured, the observer must resolve it
through Aming-owned route/precheck contracts before implementation planning and
record provider id, version, and hash evidence. Source-controlled Aming prompts,
docs, and templates must not name private provider systems or provider-specific
tool names. Nontrivial implementation is delegated to bounded `mf_sub`/worker
lanes with target files, required tests or a recorded no-test/E2E decision,
isolated worktree/fence evidence, and review evidence.
...
#### R23.1 Route Topology Selection and Worker Prompt Boundary
