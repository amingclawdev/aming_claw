...
| R24 | Route context consumption gate | `observer_led_parallel_lanes`, `mf_parallel.v1`, P0/P1 governance/routing/runtime/backlog/permission work | Route context text is not enough. Before local writes, run the local `route.pre_mutation` precheck or an equivalent `route.action_precheck` service gate. P0/cross-module/parallel implementation actions require machine route identity fields: `route_context_hash`, `prompt_contract_id`, `prompt_contract_hash`, caller role, allowed actions, blocked actions, required lanes, required evidence, and a visible injection manifest or manifest hash. Precommit and backlog-close gates still require passing timeline evidence for `route_context`, `route_action_precheck`, `bounded_implementation_worker_dispatch`, and `mf_subagent_startup`, all carrying the same route identity. Generic waivers, advisory route text, or `preflight_check` output alone do not satisfy this gate. If the route provider is unavailable, transport is closed, or route evidence is stale, mutation/write actions must block as `blocked_route_context_unavailable`. |
